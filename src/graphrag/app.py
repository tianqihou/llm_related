...
def run_query(method, query):
    command = ['python', '-m', 'graphrag.query', '--root', './ragtest', '--method', method, query]
    result = subprocess.run(command, check=True, text=True, capture_output=True)
    try:
        if method == 'global':
            response = result.stdout.split('Global Search Response:')[1]
        elif method == 'local':
            response = result.stdout.split('Local Search Response:')[1]
        print(response)
        return response
...
def save_text_as_txt(text, filename, output_dir):
    txt_file_path = os.path.join(output_dir, f"{os.path.splitext(filename)[0]}.txt")
...
def upload_file(file):
    if file is not None:
        ...
        if file_extension.lower() in ['.docx', '.pdf', '.md']:
            text = convert_to_txt(original_filename)
            txt_file_path = save_text_as_txt(text=text, filename=os.path.basename(original_filename), output_dir=os.path.dirname(original_filename))
            destination_path = os.path.join(input_dir, os.path.basename(txt_file_path))
...
def send_message(query_type, query, history, system_message, temperature, max_tokens, model):
    ...
    try:
        if query_type in ["global", "local"]:
            result = run_query(query_type, query)
        else:  # Direct chat
        ...
        with gr.Column(scale=1, elem_id="left-column"):
            with gr.Tabs():
                with gr.TabItem("Data Management"):
                    with gr.Accordion("File Upload (.txt .docx .pdf .md)", open=True):
                        file_upload = gr.File(label="Upload .txt .docx .pdf .md File", file_types=[".txt", ".docx", ".pdf", ".md"])
